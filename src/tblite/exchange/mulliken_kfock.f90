        ...
        if (allowincr) then
            if (.not.allocated(ptr%ref_D)) then
                ...
                do spin = 1, wfn%nspin
                    ...
                    & ptr%gamma_, wfn%density(:, :, spin), overlap, ptr%prev_F(:,:,spin)) ! Still have to change this to properly take 2 gammas into account
                    pot%kao(:,:,spin) = ptr%prev_F(:,:,spin)
                end do
                
                if (allocated(ptr%curr_D)) allocate(ptr%ref_D(self%nao, self%nao, wfn%nspin), source= wfn%density(:,:,:))
                ...
                do spin = 1, wfn%nspin
                    ...
                    & ptr%gamma_, ptr%curr_D(:,:,spin), overlap, pot%kao(:, :, spin)) ! Still have to change this to properly take 2 gammas into account
            ...
            do spin = 1, wfn%nspin
                call KFockSymSQM(0, 0,self%indexer, &
                & ptr%gamma_, wfn%density(:, :, spin), overlap, ptr%prev_F(:,:,spin))
                pot%kao(:, :, spin) = ptr%prev_F(:, :, spin)
            end do
